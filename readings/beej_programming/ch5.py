...
try:
    ...
    addrinfo_client = getaddrinfo(host=host_address, port=port, family=family, type=sock_type, proto=protocol)
    ...
    addrinfo_server = getaddrinfo(host=None, port=80, family=family, type=sock_type, proto=protocol, flags=AI_PASSIVE)
...
addrinfo_client = getaddrinfo(host="www.example.com", port="http", family=AF_INET, type=SOCK_STREAM, proto=0) 
addrinfo_server = getaddrinfo(host=None, port="http", family=AF_INET, type=SOCK_STREAM, proto=0, flags=AI_PASSIVE)
...
client_family, client_sock_type, client_proto, _, (client_address, client_port)  = addr_client
sock_client = socket(family=client_family, type=client_sock_type, proto=client_proto) 

server_family, server_sock_type, server_proto, _, (server_address, server_port)  = addr_server
...
if IS_UDP_RUN:
    sock_udp_client = socket(AF_INET, SOCK_DGRAM) # should not be connected to a remote server
